...
class YoutubeTranscriptReader:
    ...
    def __init__(self, model=None, model_engine=None):
        self.summary_system_prompt = os.getenv(
            'YOUTUBE_SYSTEM_MESSAGE') or YOUTUBE_SYSTEM_MESSAGE
        self.part_message_format = os.getenv(
            'PART_MESSAGE_FORMAT') or PART_MESSAGE_FORMAT
        self.whole_message_format = os.getenv(
            'WHOLE_MESSAGE_FORMAT') or WHOLE_MESSAGE_FORMAT
        self.single_message_format = os.getenv(
            'SINGLE_MESSAGE_FORMAT') or SINGLE_MESSAGE_FORMAT
        self.model = model
    ...
    def summarize(self, chunks):
        ...
        if len(chunks) > 1:
            ...
            msgs = [{
                ...
                'role': 'user',
                'content': self.whole_message_format.format(text)
            }]
        else:
            text = chunks[0]
            msgs = [{
